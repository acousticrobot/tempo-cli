Feature: Checkout Command manages the active project
  ...
  Scenario: Checkout a project by exact match
    ...
    And I successfully run `tempo checkout -e horticulture`
    Then the stdout should contain "switched to project 'horticulture'"
    And the stdout should not contain "switched to project 'horticulture - backyard bonsai'"
    And the stdout should not contain "switched to project 'horticulture - basement mushrooms'"

  Scenario: Attempting to Checkout an existing project by partial match with multiple possibilities
    Given an existing project file
    When I successfully run `tempo project horticulture`
    And I run `tempo checkout horticulture`
  ...
  Scenario: Attempting to checkout a non-existant project
    ...
    Then the stderr should contain "no projects match 'new project'"
